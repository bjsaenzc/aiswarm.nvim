#!/usr/bin/env bash
# SDD-100 demonstration from one disposable board: canonical CLI, a task journey with cancel/retry and
...
export TMUX_TMPDIR="$SOCKDIR" AISWARM_TMUX_SOCKET="demo-$$"
...
aiswarm json | head -c 200; echo
step "queue through canonical CLI"
id1=$(echo "first" | aiswarm add --title "first task"); id2=$(echo "second" | aiswarm add --title "second task" --dep "$id1"); echo "$id1 $id2"
