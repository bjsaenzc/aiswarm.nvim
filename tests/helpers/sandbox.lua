...
function M.env(root, extra)
  local e = {
    AISWARM_ROOT = root, PATH = env.PATH, HOME = env.HOME, TMPDIR = env.TMPDIR,
    ...
    AISWARM_PROVIDER = "mock", AISWARM_MOCK_SLEEP = "0",
...
--- Run the canonical aiswarm CLI (falls back to aiswarm while it does not exist).
function M.aiswarm(root, args, opts)
  opts = opts or {}
  local bin = M.bin("aiswarm") or error("no backend executable")
...
function M.legacy_board(t, name)
  ...
  local r = M.aiswarm(root, { "init" }, { env = { AISWARM_LEGACY_INIT = "1" } })
  t:eq(r.code, 0, "aiswarm init: " .. r.stderr)
  return root
end

--- Add a task with an inline prompt to a legacy board.
function M.legacy_add(t, root, args, prompt)
  local r = M.aiswarm(root, vim.list_extend({ "add" }, args), { stdin = prompt or "do the thing\n" })
  t:eq(r.code, 0, "aiswarm add: " .. r.stderr)
