...
return {
  { id = "docs.help_tags_and_links_resolve", tasks = { "SDD-096" }, suites = { "core" }, run = function(t)
    ...
    for _, f in ipairs({ "aiswarm.txt" }) do sb.write(docdir .. "/" .. f, sb.read(sb.plugin .. "/doc/" .. f)) end
    vim.cmd("helptags " .. vim.fn.fnameescape(docdir))
    local tags = sb.read(docdir .. "/tags")
    for _, tag in ipairs({ "aiswarm-setup", ":AISwarm", "aiswarm-keys", "aiswarm-compat", "aiswarm-limits" }) do t:ok(tags:find(tag, 1, true), "tag " .. tag) end
  ...
  { id = "docs.quickstart_runs_with_mock", tasks = { "SDD-096" }, suites = { "core" }, run = function(t)
    ...
    local o = sb.run({ "bash", sb.repo .. "/scripts/aiswarm-quickstart.sh" }, { env = { PATH = vim.env.PATH, HOME = vim.env.HOME, TMPDIR = vim.env.TMPDIR, AISWARM_NVIM = vim.env.AISWARM_NVIM, AISWARM_MOCK_SLEEP = "0" }, timeout = 120000 })
