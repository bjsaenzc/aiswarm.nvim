-- SDD-011: configuration/environment precedence and validation.
...
local table_driven = {
  ...
  { name = "empty_discovers", env = { AISWARM_ROOT = "" }, expect = nil, warn = false },
...
for _, row in ipairs(table_driven) do
  cases[#cases + 1] = { id = "compat.env.cli_" .. row.name, tasks = { "SDD-011" }, suites = { "core", "compatibility" }, run = function(t)
    ...
    t:eq(o.stderr:find("AISWARM_ROOT is deprecated", 1, true) ~= nil, row.warn, "legacy warning presence")
  ...
  cases[#cases + 1] = { id = "compat.env.nvim_" .. row.name, tasks = { "SDD-011" }, suites = { "core", "compatibility" }, run = function(t)
    ...
    t:defer(function() vim.env.AISWARM_ROOT = nil end)
    ...
    local warned = false
    for _, n in ipairs(seen) do if n.msg:find("AISWARM_ROOT is deprecated", 1, true) then warned = true end end
...
cases[#cases + 1] = { id = "compat.env.invalid_settings_fail_before_jobs", tasks = { "SDD-011" }, suites = { "core", "compatibility" }, run = function(t)
  ...
  t:ok(not sb.exists(dir .. "/.aiswarm"), "no files created")
end }
cases[#cases + 1] = { id = "compat.env.provider_default_is_mock", tasks = { "SDD-011", "SDD-016" }, suites = { "core", "compatibility" }, run = function(t)
  ...
  vim.env.AISWARM_PROVIDER = "codex"; config()._reset_warnings()
  t:eq(registry.default(), "codex", "canonical provider variable honored")
  vim.env.AISWARM_PROVIDER = "gemini"
  t:eq(registry.default(), "gemini", "updated provider value takes effect")
  vim.env.AISWARM_PROVIDER = nil
end }
